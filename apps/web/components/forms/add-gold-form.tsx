...
export function AddGoldForm({ onSuccess, onCancel }: AddGoldFormProps) {
  const queryClient = useQueryClient();
  const [formData, setFormData] = useState<CreateGoldInput>({
    name: "",
    quantity: 0,
    purchasePrice: 0,
  ...
  const mutation = useMutation({
    mutationFn: (data: CreateGoldInput) => goldApi.create(data),
    onSuccess: () => {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input
          ...
          placeholder="Cumhuriyet Altini, Has Altin, vb."
          required
        />
      </div>
