...
interface EditEurobondFormProps {
  eurobond: Eurobond;
  onSuccess?: () => void;
  onCancel?: () => void;
}

export function EditEurobondForm({ eurobond, onSuccess, onCancel }: EditEurobondFormProps) {
  const queryClient = useQueryClient();
  const [formData, setFormData] = useState({
    name: eurobond.name,
    isin: eurobond.isin || "",
    faceValue: Number(eurobond.faceValue),
    purchasePrice: Number(eurobond.purchasePrice),
    quantity: Number(eurobond.quantity),
    couponRate: Number(eurobond.couponRate),
    currency: eurobond.currency,
    purchaseDate: eurobond.purchaseDate.slice(0, 10),
    maturityDate: eurobond.maturityDate.slice(0, 10),
    couponFrequency: eurobond.couponFrequency,
    broker: eurobond.broker || "",
    notes: eurobond.notes || "",
  });

  const mutation = useMutation({
    mutationFn: (data: typeof formData) => eurobondsApi.update(eurobond.id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["eurobonds"] });
      queryClient.invalidateQueries({ queryKey: ["eurobonds", "summary"] });
      queryClient.invalidateQueries({ queryKey: ["dashboard", "overview"] });
      toast.success("Eurobond updated successfully");
      onSuccess?.();
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Failed to update eurobond");
    },
  });
